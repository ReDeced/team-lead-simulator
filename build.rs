...
fn main() {
    ...
    let json_file = File::open("game-data/employees.json").expect("Не удалось открыть JSON");
    ...
    let mut mpk_file = File::create("game-data/employees.mpk").expect("Не удалось создать mpk файл");
